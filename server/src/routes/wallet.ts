 ...
 */
export function setMasterXpub(xpub: string) {
  masterXpub = xpub;
}
 ...
 */
router.get('/xpub', (_req: Request, res: Response) => {
  if (!masterXpub) {
 ...
 */
router.get('/address/:index', (req: Request, res: Response) => {
  if (!masterXpub) {
    return res.status(500).json({ error: 'Master xpub not configured' });
  }
 ...
 */
router.post('/verify-address', (req: Request, res: Response) => {
  if (!masterXpub) {
    return res.status(500).json({ error: 'Master xpub not configured' });
  }
  ...
  const result = verifyAddressBelongsToMaster(
    masterXpub,
    address,
    maxIndex || 1000
  );
